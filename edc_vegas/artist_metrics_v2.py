...
def scrape_songstats():
    ...
    # Filter artists to only those not done
    artists_to_scrape = [a for a in artists if a not in processed_artists]
    
    # Apply Batch Limit
    if len(artists_to_scrape) > BATCH_SIZE:
        print(f"Batching: Selecting next {BATCH_SIZE} artists from {len(artists_to_scrape)} remaining.")
        artists_to_scrape = artists_to_scrape[:BATCH_SIZE]
    else:
        print(f"Processing remaining {len(artists_to_scrape)} artists.")

    if not artists_to_scrape:
        print("All artists have been processed! No work to do.")
        return
    ...
    with sync_playwright() as p:
        ...
        browser = p.chromium.launch(
            headless=False,
            # proxy={"server": "http://..."} # Add proxy here if desired
        ...
        for i, artist in enumerate(artists_to_scrape):
            print(f"[{i+1}/{len(artists_to_scrape)}] Processing: {artist}")
            ...
            # Save periodically
            if (i + 1) % 10 == 0:
                # Combine existing (from before script start) + new (this session) and save
                current_df = pd.DataFrame(data_results)
                combined_df = pd.concat([existing_df, current_df], ignore_index=True) if not existing_df.empty else current_df
                # Simple dedup based on Artist if needed, though exact matches shouldn't happen unless restarted mid-way without save
                # combined_df = combined_df.drop_duplicates(subset=['Artist'], keep='last')
                combined_df.to_csv(output_file, index=False)
                print(f"   -> Progress saved ({len(combined_df)} total records).")
    ...
    if len(artists_to_scrape) == BATCH_SIZE:
        print("\n[INFO] Batch complete. Run script again later to continue processing.")
    else:
        print("\nAll tasks complete.")
